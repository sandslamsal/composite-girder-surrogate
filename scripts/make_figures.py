...
def fig_parity_plots(
    ...
    fig, axes = plt.subplots(2, 2, figsize=(COL_DOUBLE_IN, COL_DOUBLE_IN * 0.85))
...
def fig_moment_curvature(
    ...
    for ax, sid in zip(axes.flat, ids):
        ...
        M_true = sub["moment_kip_in"].to_numpy() / 12.0
        phi_pred = pred["curvature_1_per_in"].to_numpy() * 1e3
        M_pred = pred["moment_kip_in"].to_numpy() / 12.0
        order = np.argsort(phi_pred)
        h0, = ax.plot(phi_true, M_true, color=COLORS[0], linewidth=1.6,
                      marker="o", markersize=3, markevery=8,
                      label="OpenSeesPy")
        h1, = ax.plot(phi_pred[order], M_pred[order], color=COLORS[1],
        ...
        ax.set_xlim(left=0)
        ymax = max(M_true.max(), M_pred.max())
...
def fig_uncertainty_band(
    ...
    std = unc["moment_kip_in_std"].to_numpy() / 12.0
    M_true = sub["moment_kip_in"].to_numpy() / 12.0
    # Sort by curvature so all lines are monotone
    order = np.argsort(phi_true)
    phi_o = phi_true[order]
    M_o = M_true[order]
    mean_o = mean[order]
    std_o = std[order]

    # Single landscape panel; the M-phi response and its ±1.96σ band
    # share one axes. The relative-uncertainty σ/|μ| panel was dropped
    # for figure-compactness — the σ values are reported in the caption
    # instead, and the legend's title carries the dropout setting.
    fig, ax = plt.subplots(figsize=(COL_SINGLE_IN, COL_SINGLE_IN * 0.85))

    h0, = ax.plot(phi_o, M_o, color=COLORS[0], linewidth=1.8, marker="o",
                  markersize=3, markevery=6, label="OpenSeesPy")
    h1, = ax.plot(phi_o, mean_o, color=COLORS[1], linestyle="--",
                  linewidth=1.8, label="Proposed model (mean)")
    h2 = ax.fill_between(
        phi_o, mean_o - 1.96 * std_o, mean_o + 1.96 * std_o,
...
def fig_error_distribution(
    ...
    # Compact one-column legend INSIDE the axes at the bottom-right
    # corner (where M's narrow violin and the lower tails of the others
    # leave a clear empty area).
